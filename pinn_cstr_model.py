...
# data prep
data = scipy.io.loadmat('cstr_data/sim_100_u.mat')  	# Load data from file
...
def trainingdata(N_u, N_f):
    ...
    all_x0_train = data_minMax[0:20000, :]
    all_x1_train = data_minMax[1:20001, 2:4]
    ...
    #     x0_train = np.vstack([x0_train, all_x0_train[i*6000+5900:(i+1):6000, :]])
    #     x1_train = np.vstack([x1_train, all_x1_train[i*6000:i*6000+100, :]])
    #     x1_train = np.vstack([x1_train, all_x1_train[i * 6000 + 5900:(i + 1):6000, :]])
...
class Sequentialmodel(tf.Module):
    ...
    def loss_PDE(self, x_to_train_f):
        ...
        f_ca = (caf_inverse - ca_inverse) - r - ca_t
        f_T = (Tf - T_inverse) - (-5960 / 500) * r - (150 / 500) * (T_inverse - Tc_inverse) - T_t
        # f_T = f_T * (K_ca/K_T)
...
N_f = 20  # Total number of collocation points
lambda1 = 0
loss_record = np.ones([1, 3])  # 记录loss变化
# Training data
xf_train, x0_train, x1_train = trainingdata(N_u, N_f)

layers = np.array([5, 32, 64, 128, 32, 16, 2])  # N hidden layers
