...
def trainingdata(N_u, N_f):
    ...
    all_x0_train = data_minMax[0:len(data_minMax)-1, 0:4]
    all_x1_train = data_minMax[1:len(data_minMax), 2:4]
    # all_x0_train = data_minMax[0:18000, 0:4]
    # all_x1_train = data_minMax[1:18001, 2:4]
...
class Sequentialmodel(tf.Module):
    ...
    def loss_PDE(self, x_to_train_f):
        ...
        with tf.GradientTape(persistent=True) as tape:
            # tape.watch(x0)
            # tape.watch(t)
            g = tf.stack([x0[:, 0], x0[:, 1],x0[:, 2],x0[:, 3]], axis=1)
        ...
        delta_ca = z_ca_inverse - ca_inverse
        delta_T = z_T_inverse - T_inverse

        r = 34930800 * tf.exp(-11843 / (1.985875 * T_inverse)) * ca_inverse
        f_ca = ((caf_inverse - ca_inverse) - r) * tau - delta_ca
        f_T = ((Tf - T_inverse) - (-5960 / 500) * r - (150 / 500) * (T_inverse - Tc_inverse)) - delta_T
...
N_f = 300  # Total number of collocation points
lambda1 = 1
loss_record = np.ones([1, 3])  # 记录loss变化
# Training data
xf_train, x0_train, x1_train = trainingdata(N_u, N_f)

layers = np.array([4, 32, 64, 128, 32, 16, 2])  # N hidden layers
