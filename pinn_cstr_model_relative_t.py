...
# data prep
data = scipy.io.loadmat('cstr_data/sim_four6000.mat')  	# Load data from file
...
class Sequentialmodel(tf.Module):
    ...
    def evaluate(self, x):
        ...
        a = x[:, 0:4]
        t = x[:, 4:5]
        A = np.zeros([len(x), layers[1]])
        A[:, layers[1]-1] = 1
        A = tf.convert_to_tensor(A)
        B = np.ones([len(x), layers[1]])
        B[:, layers[1]-1] = 0
        ...
        for i in range(len(layers) - 2):
            ...
            a = tf.nn.tanh(z)
            if i == 0:
                a = tf.add(tf.multiply(A, t), tf.multiply(B, a))
