...
class Network(keras.Model):
    ...
    def loss_PDE(self, x_to_train_f):
        ...
        with tf.GradientTape(persistent=True) as tape:
            ...
            z = self.call(g)
            ca_delta = scaler_ca.inverse_transform(z[:, 0:1]) - scaler_ca.inverse_transform(ca[:, 0:1])
            T_delta = scaler_T.inverse_transform(z[:, 1:2]) - scaler_T.inverse_transform(T[:, 0:1])
        ...
        f_ca = (caf_inverse - ca_inverse) - r - ca_delta / sample_inter
        f_T = (Tf - T_inverse) - (-5960 / 500) * r - (150 / 500) * (T_inverse - Tc_inverse) - T_delta / sample_inter
...
optimizer = tf.keras.optimizers.Adam(0.01)  # 创建优化器，指定学习率
...
compare[:, 0:1] = x[:, 0:1]
compare[:, 1:2] = data_minMax[0:sim_length, 2:3]
